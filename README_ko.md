...
8. [IEEE 대용량 크롤링 — 7차 개선](#ieee-대용량-크롤링--7차-개선)
   ...
   - 8.2 [자정 날짜 변경 시 통계 행 자동 분리](#82-자정-날짜-변경-시-통계-행-자동-분리)
9. [IEEE 대용량 크롤링 — 8차 개선](#ieee-대용량-크롤링--8차-개선)
   - 9.1 [검색 결과 감지 로직 고도화 (조기 종료 방지)](#91-검색-결과-감지-로직-고도화-조기-종료-방지)
   - 9.2 [오류 기록 세분화 (Seat Limit vs 타임아웃)](#92-오류-기록-세분화-seat-limit-vs-타임아웃)
10. [IEEE TGRS 크롤링 전체 흐름](#ieee-tgrs-크롤링-crawling_ieee_2023_2025py)
11. [빠른 시작](#빠른-시작)
12. [전체 옵션](#전체-옵션)
13. [기본 저장 경로](#기본-저장-경로)
14. [Linux 서버에서 screen으로 실행하기](#linux-서버에서-screen으로-실행하기)
15. [예상 출력](#예상-출력)
16. [예상 결과물](#예상-결과물)
17. [ScienceDirect 크롤링](#sciencedirect-크롤링)
18. [위키피디아 크롤링](#위키피디아-크롤링)
19. [토큰 수 계산](#토큰-수-계산)
20. [변경 이력](#변경-이력)
...
### 8.2 자정 날짜 변경 시 통계 행 자동 분리

#### 배경
크롤링 세션이 자정을 넘겨 여러 날짜에 걸쳐 진행될 경우, 기존에는 시작 날짜의 행에 계속 업데이트되어 `end_time`이 다음 날 시각으로 찍히는 등 통계의 왜곡이 발생하는 이슈가 있었습니다.

#### 개선 내용
- **날짜 변경 실시간 감지**: `CrawlStats.checkpoint()` 호출 시 현재 날짜를 체크하여 날짜 변경을 감지합니다.
- **통계 행 자동 분리**:
    1. **이전 날짜 마감**: 날짜 변경이 감지되면 이전 날짜의 행을 `23:59:59`로 마감하여 CSV에 저장합니다.
    2. **새 날짜 시작**: 새로운 날짜에 대해 `00:00:00`부터 시작하는 새로운 행을 CSV에 추가합니다.
- **카운터 초기화**: 새 날짜의 행이 시작될 때 `pages_processed`, `pdfs_extracted` 등의 카운터를 0부터 다시 시작하여 **일별 순수 작업량**을 정확히 측정합니다.
- **월별 파일 자동 전환**: 월이 바뀌는 시점에도 해당 월의 CSV 파일(`stats_YYYY_MM.csv`)로 자동 전환되어 저장됩니다.

#### 효과
- 일별 크롤링 성과를 정확하게 파악할 수 있습니다.
- 세션이 길어져도 날짜별 데이터가 섞이지 않아 통계 분석이 용이해집니다.

---

## IEEE 대용량 크롤링 — 8차 개선

### 9.1 검색 결과 감지 로직 고도화 (조기 종료 방지)

#### 문제
특정 페이지에서 PDF 다운로드 타임아웃이 발생한 후 다음 페이지로 넘어갔을 때, IEEE의 일시적 로딩 지연이나 프록시 병목으로 인해 결과 목록이 즉시 나타나지 않는 경우가 있습니다. 기존 로직은 이를 "데이터 없음"으로 오판하여 해당 연도 크롤링을 완전히 종료(Completed) 처리해버리는 문제가 있었습니다.

#### 개선 내용
- **확정적 종료 판정**: 단순히 아이템이 없다고 종료하지 않고, 페이지 내에 **"0 Results"** 또는 **"No results found"** 라는 명시적인 텍스트가 있을 때만 종료를 선언합니다.
- **로딩 실패 시 재시도 유도**: 아이템도 없고 "0 Results" 텍스트도 없다면 이를 "로딩 실패"로 간주합니다. 이 경우 `has_search_results`는 `True`를 반환하여 상위 루프가 "전체 선택"을 시도하게 하고, 결과적으로 페이지 실패 → 재시도 루프를 타게 하여 조기 종료를 방지합니다.

### 9.2 오류 기록 세분화 (Seat Limit vs 타임아웃)

- **정확한 원인 파악**: 기존에 모든 페이지 실패를 "Seat Limit"으로 기록하던 것을 개선하여, 실제로 페이지에 제한 메시지가 떴는지 아니면 다운로드 중 파일이 나타나지 않은 "타임아웃"인지를 구분하여 로그에 출력합니다.
- **통계 정합성**: `stats_YYYY_MM.csv`의 `download_failures` 카운트가 실제 다운로드 시도 실패 횟수를 더 정확하게 반영하게 됩니다.

### 9.3 문제 발생 연도 강제 재개 (1996년 사례) 완료

1996년 크롤링 중 p.5 다운로드 타임아웃 및 p.6 허위 빈 페이지 감지로 인해 중단되었던 문제를 해결했습니다.

1.  **원인 파악**:
    *   **p.5 실패**: IEEE 서버 부하 또는 특정 논문 묶음의 대용량으로 인해 `bulk-download.zip` 생성 및 다운로드 시작이 지연되어 타임아웃 발생.
    *   **p.6 중단**: p.5 실패 후 다음 페이지로 이동했으나, 로딩 지연으로 인해 결과가 일시적으로 보이지 않자 `has_search_results`가 이를 "마지막 페이지"로 오판하여 종료함.
2.  **조치 사항**:
    *   `has_search_results()` 로직 강화: 명시적인 "0 Results" 텍스트가 없을 경우 로딩 실패로 간주하고 재시도하도록 수정.
    *   `progress_1996.json` 상태 복구: `"status": "in_progress"` 및 `last_page_completed: 4`로 조정하여 p.5부터 재개 가능하도록 설정.
3.  **재개 방법**:
    *   아래 명령어를 실행하면 p.5부터 자동으로 재개됩니다.
    ```bash
    python crawling_ieee_2023_2025.py --headless --journal-option all --years auto --resume
    ```

---

## IEEE 대용량 크롤링 — 9차 개선 (최신)

### 10.1 빈 페이지 오판 방지 로직 최종 완성
단순히 결과 아이템(`xpl-result-item`)이 없다고 종료하는 것이 아니라, 페이지 내의 모든 텍스트를 검사하여 `no results found`, `0 results`, `returned no results` 등 **명시적인 결과 없음 메시지**가 확인될 때만 크롤링을 종료합니다. 메시지가 없는데 아이템만 없는 경우는 "로딩 지연"으로 판단하여 최대 5회 재시도 및 스크롤 트리거를 수행합니다.

### 10.2 통계 카운터(pages_skipped) 정합성 수정
페이지 실패 시 `pages_skipped` 카운터가 중복으로 증가하던 버그를 수정하여, 실제 건너뛴 페이지 수와 통계 CSV의 수치가 정확히 일치하도록 개선했습니다.
